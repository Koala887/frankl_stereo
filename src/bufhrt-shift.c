...
void usage()
{
  ...
  fprintf(stderr,
          ...
          "\n"
          "  --tcp-nodelay, -T\n"
          "      disables network Nagle's algorithm. Nagle's algorithm is designed \n"
          "      to reduce the number of small packets sent over the network by \n"
          "      buffering them until a larger packet can be sent. \n"
          "      With tcp-nodelay the written data is sent immediately.\n"
          "\n"
  ...
  if (shared)
  {
    ...
    for (i = optind; i < argc; i++)
    {
      ...
      if (shared)
      {
        ...
        if ((sems[i - optind] = sem_open(fnames[i - optind], O_RDWR)) == SEM_FAILED)
        {
        ...
        if ((semsw[i - optind] = sem_open(tmpnames[i - optind], O_RDWR)) == SEM_FAILED)
        {
        ...
        if (size == 0)
        { /* find size of shared memory chunks */
  ...
  if (interval)
  {
    ...
    while (moreinput)
    {
      ...
      for (lcount = 0, off = looperr; optr < iptr; lcount++, off += looperr)
      {
        ...
        clock_gettime(CLOCK_MONOTONIC, &mtime1);
        if ((mtime1.tv_sec > mtime.tv_sec) || (mtime1.tv_sec == mtime.tv_sec && mtime1.tv_nsec > mtime.tv_nsec))
        {
          if (verbose)
